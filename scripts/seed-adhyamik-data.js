...
const coursesData = [
  {
    ...
    what_you_receive: [
      "Detailed PDF manuals and charts",
      "Recorded meditation audios for angel alignment",
      "Lifetime access to student support group",
      "Official certification of completion"
    ],
    who_is_this_for: [
      "Spiritual seekers seeking personal transformation",
      "Reiki and yoga practitioners adding divine healing to their practice",
      "Beginners with no prior experience"
    ],
    bonuses: [
      "Free Archangel Protection Meditation audio",
      "One session review with founder Sonali"
    ],
    instructor: {
  ...
  },
  {
    ...
    what_you_receive: [
      "Exclusive Tarot workbook PDF",
      "Weekly live practice sessions",
      "Certificate of Professional Reader"
    ],
    who_is_this_for: [
      "Aspiring professional tarot readers",
      "Anyone seeking self-guidance and intuitive tools"
    ],
    instructor: {
      name: "Sonali Bhattacharya",
...
async function main() {
  ...
  const insertedCats = await mongoose.connection.db.collection("categories").insertMany(
  ...
  // Re-fetch inserted categories to construct slug -> ID mapping
  ...
  await mongoose.connection.db.collection("products").insertMany(formattedProducts);
  ...
  await mongoose.connection.db.collection("quotes").insertMany(
